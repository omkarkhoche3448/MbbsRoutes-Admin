...
const CALL_STATUS_OPTIONS = [
  ...
  { value: "CALLED", label: "Called", icon: <Phone className="h-4 w-4 text-yellow-500" /> },
  { value: "NO_RESPONSE", label: "No Response", icon: <PhoneMissed className="h-4 w-4 text-red-500" /> },
  { value: "CALLBACK_REQUESTED", label: "Callback Requested", icon: <Phone className="h-4 w-4 text-blue-500" /> },
  { value: "COMPLETED", label: "Completed", icon: <PhoneCall className="h-4 w-4 text-green-500" /> },
];

export function CallStatusDialog({ isOpen, onClose, student, onStatusUpdated }) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the call status for {student?.name || "this student"}
          </DialogDescription>
        </DialogHeader>
